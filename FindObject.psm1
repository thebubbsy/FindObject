function Find-ObjectByName {
    ...
    begin {
        ...
        $FinderLogic = $logic
        $FinderPatterns = @(foreach ($k in $FinderKeywords) { "*$k*" })
        ...
        if ($FinderLogic -eq 'OR') {
            ...
            for ($i = 0; $i -lt $patternCount; $i++) {
                ...
                if ($objectName -like $pattern) {
                    Write-Verbose "OR match found for pattern in name"
                    Write-Output $InputObject
                    return # Found one match, output and exit process block
                }
            }
        } else {
            # Logic is AND
            ...
            if ($patternCount -eq 0) {
                ...
                for ($i = 0; $i -lt $patternCount; $i++) {
                    ...
                    if ($objectName -notlike $pattern) {
                        Write-Verbose "AND condition failed: pattern not found in name"
                        return # Found one keyword that doesn't match, exit process block immediately
                    }
                }
            }
            # If it made it through the loop, it matched all patterns
            Write-Verbose "AND condition met for object name"
            Write-Output $InputObject
        }
    }
