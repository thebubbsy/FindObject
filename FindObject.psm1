function Find-ObjectByName {
    ...
    begin {
        ...
        $FinderKeywords = [string[]]::new($keywords.Count)
        # Pre-calculate wildcard patterns to avoid per-object string interpolation in the process block
        for ($i = 0; $i -lt $keywords.Count; $i++) {
            $FinderKeywords[$i] = "*$($keywords[$i])*"
        }
        $FinderLogic = $logic
    ...
    process {
        ...
        # Ensure it's a string before doing string operations. Faster than .ToString()
        ...
        if ($FinderLogic -eq 'OR') {
            ...
            for ($i = 0; $i -lt $FinderKeywordCount; $i++) {
                if ($objectName -notlike $FinderKeywords[$i]) {
                    $match = $false
                    break # Found one keyword that doesn't match
                }
            }
            if ($match) {
                Write-Output $InputObject
