function Find-ObjectByName {
    ...
    begin {
        ...
        # Optimization: Pre-calculate wildcard patterns to avoid string interpolation in high-throughput process block
        $FinderKeywords = @($keywords | ForEach-Object { "*$_*" })
        $FinderKeywordsCount = $FinderKeywords.Length
        $FinderLogic = $logic
    ...
    process {
        ...
        # Optimization: Fast type conversion instead of .ToString() to prevent null reference exceptions
        ...
        if ($FinderLogic -eq 'OR') {
            ...
            # Optimization: Standard for loop with cached length is faster than foreach
            for ($i = 0; $i -lt $FinderKeywordsCount; $i++) {
                if ($objectName -like $FinderKeywords[$i]) {
                    Write-Verbose "OR match found for pattern '$($FinderKeywords[$i])' in name '$objectName'"
                    # Optimization: Early return skips maintaining $match variable and outputting below
            ...
            for ($i = 0; $i -lt $FinderKeywordsCount; $i++) {
                if ($objectName -notlike $FinderKeywords[$i]) {
                    Write-Verbose "AND condition failed: pattern '$($FinderKeywords[$i])' not found in name '$objectName'"
                    # Optimization: Early return skips maintaining $match variable
                    return
                }
            }

            # If we made it here without returning, all AND conditions passed
