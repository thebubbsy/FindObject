function Find-ObjectByName {
    ...
    begin {
        ...
        $patternCount = $keywords.Count
        $FinderPatterns = [string[]]::new($patternCount)
        ...
        if ($FinderLogic -eq 'OR') {
            ...
            for ($i = 0; $i -lt $patternCount; $i++) {
                if ($objectName -like $FinderPatterns[$i]) {
                    ...
                    return # Found one match, early return outputs object and skips remaining checks
                }
            }
        } else {
            ...
            # If we get here, all AND conditions matched
            Write-Output $InputObject
        }
    }
