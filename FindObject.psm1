function Find-ObjectByName {
    ...
    begin {
        ...
        # Pre-calculate wildcard patterns to avoid string interpolation in process block
        $FinderPatterns = @($FinderKeywords | ForEach-Object { "*$_*" })
    ...
    process {
        ...
        # Ensure it's a string before doing string operations. Faster than always calling .ToString()
        ...
        if ($FinderLogic -eq 'OR') {
            ...
            for ($i = 0; $i -lt $FinderPatternsCount; $i++) {
                if ($objectName -like $FinderPatterns[$i]) {
                    ...
                    return # Early return to output quickly and avoid further processing
                }
            }
            ...
            for ($i = 0; $i -lt $FinderPatternsCount; $i++) {
                if ($objectName -notlike $FinderPatterns[$i]) {
                    ...
                    Write-Verbose "Object '$objectName' did not pass the filter."
                    return # Early return skips remaining AND checks if one fails
                }
                Write-Verbose "AND condition met (so far): keyword '$($FinderKeywords[$i])' found in name '$objectName'"
            }

            # If we haven't returned yet, all AND conditions met
            Write-Verbose "Object '$objectName' passed the filter. Outputting."
            Write-Output $InputObject
        }
    }
