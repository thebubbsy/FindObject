function Find-ObjectByName {
    ...
    begin {
        ...
        # ⚡ Bolt Optimization: Pre-calculate wildcard patterns in begin block
        # This avoids expensive string interpolation ("*$keyword*") inside the process loop
        $FinderPatterns = [string[]]::new($FinderKeywords.Length)
        for ($i = 0; $i -lt $FinderKeywords.Length; $i++) {
    ...
    process {
        ...
        # ⚡ Bolt Optimization: Use -isnot [string] check and cast instead of .ToString()
        ...
        # ⚡ Bolt Optimization: Iterate using standard for loop over pre-cached lengths
        # and use early returns to exit process block instead of maintaining $match state
