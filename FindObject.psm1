function Find-ObjectByName {
    ...
    begin {
        ...
        # ⚡ Bolt Optimization: Pre-calculate wildcard patterns once in 'begin'
        # instead of interpolating strings like "*$keyword*" inside the high-frequency 'process' block.
    ...
    process {
        ...
        # ⚡ Bolt Optimization: Using -isnot [string] and casting is faster than calling .ToString() on arbitrary objects
        ...
        # ⚡ Bolt Optimization: Use 'for' loop with pre-cached count and pre-calculated patterns,
        # plus early return directly to Write-Output, skipping the state tracking variable '$match'.
        if ($FinderLogic -eq 'OR') {
            ...
            for ($i = 0; $i -lt $PatternCount; $i++) {
                if ($objectName -notlike $FinderPatterns[$i]) {
                    ...
                    return # Found one keyword that doesn't match, early return
                }
            }
